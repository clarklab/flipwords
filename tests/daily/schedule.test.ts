...
describe('getSessionForDate', () => {
  ...
  it('NEVER rewrites history — pinned sessions for already-played dates', () => {
    ...
    for (const [date, ids] of Object.entries(pinned)) {
      expect(getSessionForDate(date).map((l) => l.id), date).toEqual(ids)
    }
  })
